...
def load_stopwords(path='e:/data/stopwords/stopwords.txt'):
    """
    加载停用词
    :param path:
    :return:
    """
    with open(path) as f:
        stopwords = [line.rstrip() for line in open(path, 'r').readlines()]
    stopwords.extend([' ', '\t', '\n'])
...
s=open('e:/data/yingyan/cluster/res-1','r',encoding='utf-8').read()
