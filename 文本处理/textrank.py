...
def handel_data():
    ...
    for line in fp.readlines():    # 按行处理
        ...
        for seg_line in seg_lines:   # 判断如果是名词则保留
                if seg_line.flag == 'l' or seg_line.flag == 'nr' or seg_line.flag == 'ns' or seg_line.flag == 'nt' or seg_line.flag == 'nz':
...
def build_matrix():
    ...
    ########第四步，将排名前100的关键词写入文件
    fp_textrank_result = open('e:/data/yingyan/Label_textrank.txt', 'w+')
    for i in range(100):
        fp_textrank_result.write(key_words[i][0] + ' ' + str(round(key_words[i][1], 10)))
        fp_textrank_result.write('\n')
    fp_textrank_result.close()
    """
    fp_test = open('f://emotion/mysite/Label_extract/url分类.txt', 'w+')
    for i in range(100):
...
def post_handel_textrank():
    # 读取排名前100的词和权值
    fp_result = open('e:/data/yingyan/Label_textrank.txt', 'r')
    ...
    fp_result.close()
    #读取原始微博连成字符串
    fp_weibo = open('e:/data/不良事件.txt', 'rb')
    weibo_list = []
    ...
    for i in range(100):
        for j in range(i, 100):
            ...
            for sent in weibo_list:
                str1_count += sent.count(str1.encode())
                str2_count += sent.count(str2.encode())
